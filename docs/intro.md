    ...
    <div className="card padding--lg connect-overview-card">
      <a className="connect-overview-card__link" href={useBaseUrl('/docs/core-capabilities/daypart-automation')}>
        <div className="connect-overview-card__content">
          ...
          <img className="connect-overview-card__image" src={useBaseUrl('/img/DayPart.svg')} alt="Daypart automation icon" />
        </div>
      </a>
    </div>
  </div>
...
Access Connect securely through the Login screen to manage your estate, monitor live performance, and control operations from a single cloud platform.

### Manager

Manager login provides centralized access for configuring estate-wide settings, schedules, and reporting.

<div style={{textAlign: 'center'}}>
  <img src={useBaseUrl('/img/ManagerLogin.gif')} alt="Manager login walkthrough" style={{maxWidth: '520px', width: '100%'}} />
</div>

### Store Manager

Store Manager login focuses on site-level control, daily checks, and local operational adjustments.

### Operator

Operator login provides a streamlined interface for frontline actions and real-time task execution.
